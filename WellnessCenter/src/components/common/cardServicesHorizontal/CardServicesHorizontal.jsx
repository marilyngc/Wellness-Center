...
export const CardServicesHorizontal = ({ title, img, duration, items,review }) => {
  ...
  return (
      <div className="d-flex row align-items-center mx-lg-5 mx-4 my-3  border shadow rounded cardContent" id={title}>
        ...
        <div className=" col  d-flex flex-column cardBody ">
          <div className="px-lg-5 px-1 pt-lg-5 pt-4 col ">
            ...
            <>
              <p>{duration}</p>
              <ul>
                {items.length == 1 ? <h6>{items[0]}</h6> : items.map((item, index) => <li key={index}>{item}</li>)}
              </ul> 
            </>
          ):(
